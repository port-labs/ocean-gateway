...
- **Event TTL** (`EVENT_TTL`, default 2h): age-based trim via
  `MINID` on each `XADD`, keeping roughly the last 2 hours of events per stream.
  Set to `0` to disable age trim.
...
| `REDIS_LIVE_EVENTS_URL` | `localhost:6379` | Redis address |
| `REDIS_LIVE_EVENTS_USERNAME` | _(empty)_ | Redis username (ACL) |
| `REDIS_LIVE_EVENTS_PASSWORD` | _(empty)_ | Redis password |
| `REDIS_LIVE_EVENTS_ENABLE_TLS` | `false` | Enable TLS for Redis |
...
| `EVENT_TTL` | `2h` | Trim stream entries older than this via `MINID` (`0` = no age trim) |
...
- **`EVENT_TTL`** (default `2h`) trims individual entries via
  `XADD MINID` on every write. Set to `0` to disable; consumers that lag past
  the TTL may miss unacked messages.
